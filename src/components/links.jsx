...
class Links extends Component {
    ...
    prepareLinkListing() {
        return (
            <div className="container bg-gray mt-5">
                ...
                {!this.props.isLoading && this.props.links.map((link) => <div className="columns" key={link._id}>
                    <div className="column col-xs-9">{link.title}</div>
                    <div className="column col-xs-2">{link.body}</div>
                    <div className="column col-xs-1">
                        <button onClick={() => this.props.deleteLink(link._id)} className={this.props.delLinkId === link._id
                            ? "btn loading disabled"
                            : "btn btn-primary btn-block"}>
                            {this.props.delLinkId === link._id
                                ? "Deleting"
                                : "Delete"}
                        </button>
                    </div>
    ...
    render() {
        var btnClass = this.props.isLoading
            ? "btn centered btn-primary loading"
            : "btn centered btn-primary"
        return (
